  ...
  return (
    <div>
      ...
      <div
        ...
        className="batch-form-header"
      >
        <div>
          <label className="small" htmlFor="batchName">
            Batch Name:
          </label>
          <input
            id="batchName"
        ...
        <div>
          <label className="small" htmlFor="batchDescription">
            Description:
          </label>
          <input
            id="batchDescription"
        ...
        <div>
          <label className="small" htmlFor="batchStart">
            Start Date:
          </label>
          <input
            id="batchStart"
        ...
        <div>
          <label className="small" htmlFor="batchDue">
            Due Date:
          </label>
          <input
            id="batchDue"
      ...
      >
        ...
        <label
          className="small modal-selector-toggle"
        ...
        >
          <input
            ...
            onChange={(e) => {
              ...
              try {
                localStorage.setItem(modalToggleKey, e.target.checked ? 'true' : 'false');
              } catch {}
            }}
          />
          Use modal selectors
      ...
      {!useModalSelectors ? (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 24, marginBottom: 24 }}>
          ...
          {importBusy && (
            <div
              className="small import-progress-banner"
              style={{
                background: '#fff8e1',
            ...
            >
              ...
              <div className="progressBar" aria-hidden="true" style={{ marginTop: 6 }}>
                <i
                  style={{
                    width: `${importTotal ? Math.round((importDone / importTotal) * 100) : 0}%`,
                  }}
                />
              </div>
              {importRows.length > 0 && (
          ...
          <div className="sharepoint-picker-section">
            <SharePointBrowser
              ...
              onDocumentSelect={async (spDocs) => {
                if (props.importBusy) {
                  showToast('An import is already running', 'warning');
                  return;
                }
                ...
                try {
                  ...
                  props.setImportBusy(true);
                  props.setImportTotal(spDocs.length);
                  props.setImportDone(() => 0);
                  props.setImportRows(() => []);
                  ...
                  for (const d of spDocs) {
                    ...
                    try {
                      const res = await fetch(`${base}/api/library/save-graph`, {
                        method: 'POST',
                        headers: {
                      ...
                      const doc: SimpleDoc = {
                        ...
                        itemId,
                        source: 'sharepoint',
                        localFileId: j?.id ?? null,
                        localUrl: localUrl || null,
                      };
                      imported.push(doc);
                      props.setImportRows((rows) => [
                        ...rows,
                        { name, status: j?.deduped ? 'deduped' : 'saved' },
                      ]);
                  ...
                  setBatchForm((prev) => ({
                    ...prev,
                    selectedDocuments: props.mergeDocuments(prev.selectedDocuments, imported),
                  }));
                  showToast(
                    `Imported ${imported.length - failed} • deduped ${dedupedCount}${failed ? ` • failed ${failed}` : ''}`,
                    failed ? 'warning' : 'success'
                  );
                } catch (e) {
                  setBatchForm((prev) => ({
                    ...prev,
                    selectedDocuments: props.mergeDocuments(
          ...
          <div className="card batch-documents-section" style={{ padding: 16 }}>
            ...
            {batchForm.selectedDocuments.length === 0 ? (
              <div className="small muted">
                No documents selected yet. Use Library or SharePoint above.
              </div>
            ) : (
                  ...
                  >
                    <div style={{ minWidth: 0 }}>
                      ...
                      >
                        ...
                        {d.source === 'sharepoint' && d.localUrl && (
                          <span className="badge" title="Server backup created">
                            backed up
                          </span>
                        )}
                        {(d.localUrl || d.url) && (
                          <a
                            href={(d.localUrl || d.url)!}
                            target="_blank"
                            rel="noreferrer"
                            className="small"
                          >
                    ...
                    <button
                      ...
                      onClick={() => props.removeSelectedDoc(idx)}
          ...
          <div className="card business-mapping-section" style={{ padding: 16 }}>
            ...
            >
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                ...
                <select
                  value={String(defaultBusinessId)}
                  onChange={(e) => setDefaultBusinessId(e.target.value ? e.target.value : '')}
                >
                  <option value="">—</option>
                  {businesses.map((b) => (
                ...
                <select
                  onChange={(e) => applyBusinessToAll(e.target.value ? e.target.value : null)}
                  defaultValue=""
                >
                  <option value="">—</option>
                  {businesses.map((b) => (
                ...
                {batchForm.selectedGroups.length > 0 && (
                  <button
                    className="btn ghost sm"
                    onClick={expandGroupsForMapping}
                    title="Load members of selected groups for per-user mapping"
                  >
                    Expand groups for mapping
                  </button>
                )}
            ...
            {sqliteEnabled && businesses.length === 0 && (
              <div className="small muted">No businesses found.</div>
            )}
            {sqliteEnabled && businesses.length > 0 && (
                  ...
                  return (
                    ...
                    >
                      ...
                      <select
                        value={String(sel)}
                        onChange={(e) =>
                          props.setUserBusiness(email, e.target.value ? e.target.value : null)
                        }
                      >
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 16, marginBottom: 24 }}>
          ...
          <div
            className="card batch-users-section"
            style={{
              padding: 16,
              display: 'flex',
              justifyContent: 'space-between',
          ...
          >
            <div>
              ...
              <div className="small muted">
                {batchForm.selectedUsers.length} users, {batchForm.selectedGroups.length} groups
                selected
              </div>
            </div>
            <button className="btn sm" onClick={() => setUsersModalOpen(true)}>
              Edit selection
            </button>
          </div>
          ...
          <div
            className="card documents-summary-card"
            style={{
              padding: 16,
              display: 'flex',
              justifyContent: 'space-between',
          ...
          >
            <div>
              ...
              <div className="small muted">
                {batchForm.selectedDocuments.length} document(s) selected
              </div>
            </div>
            <button className="btn sm" onClick={() => setDocsModalOpen(true)}>
              Choose documents
            </button>
          </div>
          ...
          <div className="card business-mapping-section" style={{ padding: 16 }}>
            ...
            {sqliteEnabled && businesses.length > 0 ? (
              ...
              <div className="small muted" style={{ marginTop: 8 }}>
                {!sqliteEnabled ? 'Enable SQLite to load businesses.' : 'No businesses found.'}
              </div>
            )}
          </div>
        </div>
      ...
      <div
        ...
        style={{ backgroundColor: '#f8f9fa', padding: 16, borderRadius: 8 }}
      >
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16 }}>
          <div className="summary-assigned">
            ...
            <div style={{ fontWeight: 'bold' }}>
              {batchForm.selectedUsers.length +
                batchForm.selectedGroups.reduce(
          ...
          <div className="summary-duration">
            ...
            <div style={{ fontWeight: 'bold' }}>
              {batchForm.startDate && batchForm.dueDate
                ? Math.ceil(
                    (new Date(batchForm.dueDate).getTime() -
        ...
        >
          <label style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <input
              id="notifyByEmail"
              type="checkbox"
              checked={batchForm.notifyByEmail}
              onChange={(e) => setBatchForm({ ...batchForm, notifyByEmail: e.target.checked })}
            />
            <span className="small">Email notification (Microsoft Graph)</span>
          </label>
          <label
            style={{ display: 'flex', alignItems: 'center', gap: 8, opacity: 0.6 }}
            title="Requires Teams Chat.ReadWrite; coming soon"
          >
            <input
              id="notifyByTeams"
              type="checkbox"
              checked={batchForm.notifyByTeams}
              onChange={(e) => setBatchForm({ ...batchForm, notifyByTeams: e.target.checked })}
              disabled
            />
        ...
        <div style={{ marginTop: 16, display: 'flex', gap: 8 }}>
          <button
            ...
            disabled={
              ...
              !batchForm.dueDate ||
              batchForm.selectedDocuments.length === 0 ||
              (batchForm.selectedUsers.length === 0 && batchForm.selectedGroups.length === 0)
            }
          >
          ...
          <button
            id="preview-recipients-button"
            className="btn ghost"
            title="Preview expanded recipients"
                ...
                if (batchForm.selectedGroups.length > 0) {
                  ...
                  const arrays = await Promise.all(
                    batchForm.selectedGroups.map((g) =>
                      getGroupMembers(token, (g as any).id).catch(() => [])
                    )
                  );
